...
import { Extension, ExtensionConstructor, HttpRequestInfo } from './extension';
...
type TrackedXMLHttpRequest = XMLHttpRequest & {
  __xhsExporterRequestBody?: unknown;
};
...
function getFetchRequestInfo(resource: RequestInfo | URL, config?: RequestInit) {
  ...
  const body = normalizeRequestBody(config?.body);
...
export class ExtensionManager {
  ...
  private installHttpHooks() {
    ...
    globalObject.XMLHttpRequest.prototype.open = function (method: string, url: string) {
      ...
      this.addEventListener('load', () => {
        ...
        manager.dispatchInterceptors(
          {
            method,
            url,
            body: normalizeRequestBody((this as TrackedXMLHttpRequest).__xhsExporterRequestBody),
          },
          this,
        );
      });
    ...
    globalObject.XMLHttpRequest.prototype.send = function (
      body?: Document | XMLHttpRequestBodyInit | null,
    ) {
      (this as TrackedXMLHttpRequest).__xhsExporterRequestBody = body;
    ...
    globalObject.fetch = async (...args) => {
      ...
      const requestInfo = getFetchRequestInfo(resource, config);
      ...
      if (response.ok) {
        ...
        clone
          ...
          .then((responseText) => {
            ...
            manager.dispatchInterceptors(requestInfo, fakeXhr);
          })
          .catch((err) => {
  ...
  private dispatchInterceptors(request: HttpRequestInfo, response: XMLHttpRequest) {
    this.getExtensions()
      .filter((ext) => ext.enabled)
      .forEach((ext) => {
        const func = ext.intercept();
        if (func) {
          func(request, response, ext);
        }
      });
  }
}
