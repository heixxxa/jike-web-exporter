...
export class DatabaseManager {
  ...
  */
  ...
  private jikePosts() {
    return this.db.table<JikePost>('jike_posts');
  }

  private jikeComments() {
    return this.db.table<JikeComment>('jike_comments');
  }
  ...
  */
  ...
  async extGetCapturedJikePosts(extName: string) {
    const captures = await this.extGetCaptures(extName);
    if (captures.length === 0) {
      return [];
    }

    const postIds = captures.map((capture) => capture.data_key);
    const posts = await this.jikePosts()
      .where('post_id')
      .anyOf(postIds)
      .toArray()
      .catch((error) => {
    ...
    return this.mergeCaptureMetadata(captures, posts, (post) => post.post_id);
  }

  async extGetCapturedJikeComments(extName: string) {
    const captures = await this.extGetCaptures(extName);
    if (captures.length === 0) {
      return [];
    }
    ...
    const commentIds = captures.map((capture) => capture.data_key);
    const comments = await this.jikeComments()
      .where('comment_id')
      .anyOf(commentIds)
      .toArray()
      .catch((error) => {
  ...
  */
  ...
  async extAddJikePosts(extName: string, posts: JikePost[]) {
    if (posts.length === 0) return;

    const capturedAt = Date.now();
    await this.jikePosts().bulkPut(posts);
    await this.upsertCaptures(
      posts.map((post) => ({
        id: `${extName}-${post.post_id}`,
        extension: extName,
        type: ExtensionType.JIKE_POST,
        data_key: post.post_id,
        created_at: capturedAt,
      })),
    );
  }

  async extAddJikeComments(extName: string, comments: JikeComment[]) {
    if (comments.length === 0) return;

    const capturedAt = Date.now();
    await this.jikeComments().bulkPut(comments);
    await this.upsertCaptures(
      comments.map((comment) => ({
        id: `${extName}-${comment.comment_id}`,
        extension: extName,
        type: ExtensionType.JIKE_COMMENT,
        data_key: comment.comment_id,
        created_at: capturedAt,
      })),
    );
  ...
  */
  ...
  async count() {
    try {
      return {
        xhs_notes: await this.xhsNotes().count(),
        xhs_comments: await this.xhsComments().count(),
        jike_posts: await this.jikePosts().count(),
        jike_comments: await this.jikeComments().count(),
  ...
  */
  ...
  async deleteAllJikePosts() {
    return this.jikePosts().clear().catch(this.logError);
  }

  async deleteAllJikeComments() {
    return this.jikeComments().clear().catch(this.logError);
  }
  ...
  */
  ...
  async init() {
    ...
    const jikePostIndexPaths: KeyPaths<JikePost>[] = [
      'post_id',
      'user.user_id',
      'like_count',
      'comment_count',
    ...
    const jikeCommentIndexPaths: KeyPaths<JikeComment>[] = [
      'comment_id',
      'target_id',
      'user.user_id',
      'like_count',
    ...
    try {
      this.db
        ...
        .stores({
          ...
          jike_posts: jikePostIndexPaths.join(','),
          jike_comments: jikeCommentIndexPaths.join(','),
        })
        .upgrade(async () => {
